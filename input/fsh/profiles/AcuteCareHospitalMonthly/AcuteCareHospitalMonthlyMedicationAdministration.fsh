...
* reasonReference MS
  * reference 1.. MS
* reasonReference only Reference(AcuteCareHospitalMonthlyReportingCondition)
* request MS
...
* dosage 1.. MS
  ...
  * dose ^short = "(QI-Core) Amount of medication per dose"
  ...
  // TODO QUESTION ON DD, QI Core MedicationAdministration does not allow not-done code. THis means we will need to be based on FHIR base and add the combo constraints of the done and not done profiles
