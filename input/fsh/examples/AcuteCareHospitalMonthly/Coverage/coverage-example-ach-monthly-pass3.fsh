Instance: coverage-example-ach-monthly-pass3
InstanceOf: AcuteCareHospitalMonthlyReportingCoverage
Title: "Coverage - Example ACH Monthly Pass3"
Description: "Coverage - Example ACH Monthly Pass3 - Commercial PPO"
Usage: #example
* identifier.type = $v2-0203#MB "Member Number"
* identifier.system = "http://example.org/fhir/memberidentifier"
* identifier.value = "0945345"
* status = #active
* type = $v3-ActCode#PPO "preferred provider organization policy"
* subscriberId = "7843574536"
* beneficiary = Reference(patient-example-ach-monthly-pass3)
* relationship = $subscriber-relationship#self "Self"
* payor.display = "Nation First Health Care"
//* payor.reference = "http://example.org/fhir/Organization/NationFirst"
* payor = Reference(organization-coverage-example-ach-monthly-nationfirst)
